...
class AssetVideoScrollView: UIScrollView {
    ...
    var trimDurationSec: Double = 60 //スクロールビューのスケールを変えられる
    ...
    private func getThumbnailFrameSize(from asset: AVAsset) -> CGSize? {
        ...
        if transform.a == 0.0 {
            ratio = assetSize.height / assetSize.width
        } else {
    ...
    private func setContentSize(for asset: AVAsset) -> CGSize {
        ...
        let contentWidthFactor = CGFloat(max(1, asset.duration.seconds / trimDurationSec))
